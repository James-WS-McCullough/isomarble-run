  ...
  switch (trackType) {
    ...
    default:
      ...
      return defaultDirections[
        Math.floor(Math.random() * defaultDirections.length)
      ];
  }
}
...
function getRollVelocity(direction: string): { x: number; y: number } {
  switch (direction) {
    case "up-right":
      return {
        x: PHYSICS.ROLLING_SPEED * 0.5,
        y: -PHYSICS.ROLLING_SPEED * 0.5,
      };
    case "down-right":
...
export function updateMarble(
  ...
  switch (marble.state) {
    case "falling":
      ...
      newMarble.position.x =
        (newMarble.screenPosition.x / 50 + newMarble.screenPosition.y / 25) / 2;
      newMarble.position.y =
        (newMarble.screenPosition.y / 25 - newMarble.screenPosition.x / 50) / 2;
    ...
    case "rolling":
      ...
      if (!currentTrack) {
        ...
        const lastPos = {
          x:
            Math.round(
              (newMarble.screenPosition.x - marble.velocity.x) / 50 +
                (newMarble.screenPosition.y - marble.velocity.y) / 25
            ) / 2,
          y:
            Math.round(
              (newMarble.screenPosition.y - marble.velocity.y) / 25 -
